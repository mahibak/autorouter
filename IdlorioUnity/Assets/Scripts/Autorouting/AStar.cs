...
class AStar
{
    ...
    int width;
    int height;
    Func<global::Point, float> costEvaluator;
    ...
    public AStar(int width, int height, Func<global::Point, float> costEvaluator)
    {
        this.width = width;
        this.height = height;
    ...
    public List<global::Point> Find(int startX, int startY, int goalX, int goalY)
    {
        ...
        while(openSet.Count > 0)
        {
            ...
            foreach (Point neighbor in GetNeighbors(current))
            {
                ...
                float tentativeGScore = current.gScore + costEvaluator(new global::Point(neighbor.X, neighbor.Y));
    ...
    private float distanceBetween(Point current, Point neighbor)
    {
        ...
        return (float)System.Math.Sqrt(distX * distX + distY * distY);
    }

    private List<global::Point> reconstructPath(Point current)
    {
        List<global::Point> totalPath = new List<global::Point>();
        totalPath.Add(new global::Point(current.X, current.Y));

        while(current.cameFrom != null)
        {
            totalPath.Insert(0, new global::Point(current.cameFrom.X, current.cameFrom.Y));
